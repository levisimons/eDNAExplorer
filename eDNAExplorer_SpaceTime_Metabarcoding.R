...
tryCatch(
  {
    ...
    result <- process_metadata(
      ...
      environmental_variable = c(categorical_variables, continuous_variables, field_vars, "sample_id")
    )
    ...
    print("Merging relative abundance results into data filtered by reads per sample and mismatches and then filtered on relative abundances.")
    tronko_input <- tronko_input %>%
      left_join(tronko_unfiltered, na_matches = "never") %>%
      filter(freq >= filter_threshold) %>%
      select(-freq)
    tronko_db <- as.data.frame(tronko_input)
    print("Removing taxa which are unknown at a given rank.")
    ...
    print("Filtering results by species list.")
    ...
    print("Reading in Taxonomy output and filtering it.")
    ...
    print("Determining the number of samples passing the filter. (TEST)")
    # Determine the number of samples passing the filter.
    # Merge in taxonomic data.
    if (nrow(tronko_db) > 0) {
      print("Getting num filtered results.")
      num_filtered_samples <- length(unique(tronko_db$SampleID))

      print("Merge in taxonomic data.")
    ...
    print("Merge Tronko output with sample metadata")
    ...
    print("Generate the number of samples and number of samples post-filtering as a return object.")
    ...
    print("Aggregate merged data by the appropriate time interval to find taxa presence by time.")
    # Aggregate merged data by the appropriate time interval to find taxa presence by time.
    project_db_by_time <- project_db %>%
      dplyr::distinct(site, SampleID, sample_date, Latin_Name, .keep_all = TRUE) %>%
      dplyr::group_by(sample_date, Latin_Name) %>%
      dplyr::summarise(n = n_distinct(SampleID)) %>%
    ...
    project_db_by_site <- project_db %>%
      dplyr::distinct(site, SampleID, Latin_Name, .keep_all = T) %>%
    ...
    # Aggregate merged data by the appropriate time interval to find taxa presence by time.
    project_db_by_site_time <- project_db %>%
      dplyr::distinct(site, SampleID, sample_date, Latin_Name, .keep_all = T) %>%
  ...
  error = function(e) {
    print("Error in eDNAExplorer_SpaceTime_Metabarcoding.R:")
    print(e$message)
