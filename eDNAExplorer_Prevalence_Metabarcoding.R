...
write(toJSON(list(generating = TRUE, lastRanAt = Sys.time())),filename)
...
Metadata <- Metadata %>% filter(projectid == Project_ID) %>% filter(!is.na(latitude) & !is.na(longitude))
...
if(TaxonomicRank!="kingdom"){
  ...
  TaxonomyInput <- TaxonomyInput %>% filter(rank == TaxonomicRank) %>% filter(Taxon %in% TaxaList) %>% select(TaxonomicRanks[2:TaxonomicNum],Taxon,Common_Name,Image_URL)
  }
