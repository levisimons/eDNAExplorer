...
process_error <- function(e, filename = "error.json") {
  ...
  s3_path <- if (is.null(Taxon_name) || Taxon_name == "") {
    paste("s3://",bucket,"/errors/timelines/", new_filename," --endpoint-url ",ENDPOINT_URL, sep = "")
  } else {
    paste("s3://",bucket,"/timelines/error_", dest_filename, " --endpoint-url ",ENDPOINT_URL, sep = "")
  }
...
tryCatch(
  {
    ...
    system(paste("aws s3 cp ", filename, " s3://",bucket,"/timelines/", dest_filename, " --endpoint-url ",ENDPOINT_URL, sep = ""), intern = TRUE)
    system(paste("rm ", filename, sep = ""))
    ...
    system(paste("aws s3 cp ",filename," s3://",bucket,"/timelines/",filename," --endpoint-url ",ENDPOINT_URL,sep=""),intern=TRUE)
    system(paste("rm ",filename,sep=""))
