...
if (length(args) != 12) {
  stop("Need the following inputs: ProjectID, First_Date, Last_Date, Marker, Num_Mismatch, TaxonomicRank, CountThreshold, FilterThreshold, SpeciesList, EnvironmentalParameter, AlphaDiversity, Sites.", call. = FALSE)
...
process_error <- function(e, filename = "error.json") {
  ...
  s3_path <- if (is.null(ProjectID) || ProjectID == "") {
    paste("s3://",bucket,"/errors/alpha/", new_filename," --endpoint-url ",ENDPOINT_URL, sep = "")
  } else {
...
# Rscript --vanilla eDNAExplorer_Alpha_Metabarcoding.R "ProjectID" "First_Date" "Last_Date" "Marker" "Num_Mismatch" "TaxonomicRank" "CountThreshold" "FilterThreshold" "SpeciesList" "EnvironmentalParameter" "AlphaDiversity" "Sites"
